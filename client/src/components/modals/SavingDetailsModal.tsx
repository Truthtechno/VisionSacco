...
interface SavingDetailsModalProps {
  isOpen: boolean;
  onClose: () => void;
  saving: DepositWithDetails | null;
}

export default function SavingDetailsModal({ isOpen, onClose, saving }: SavingDetailsModalProps) {
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-500">Saving Number</label>
                  <p className="text-lg font-semibold">{saving.depositNumber}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Amount</label>
                  <p className="text-lg font-semibold text-emerald-600">{formatCurrency(saving.amount)}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Payment Method</label>
                  <p className="text-lg capitalize">{saving.depositMethod.replace('_', ' ')}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Status</label>
                  <div className="mt-1">{getStatusBadge(saving.status)}</div>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-lg">{saving.memberName}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Member Number</label>
                  <p className="text-lg">{saving.memberNumber}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-4">
                <div>
                  ...
                  <p className="text-lg">{formatDate(saving.createdAt)}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Recorded By</label>
                  <p className="text-lg">{saving.recordedByName}</p>
                </div>
                {saving.approvedAt && saving.approvedByName && (
                  <>
                    <div>
                      <label className="text-sm font-medium text-gray-500">
                        {saving.status === "approved" ? "Approved Date" : "Rejected Date"}
                      </label>
                      <p className="text-lg">{formatDate(saving.approvedAt)}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-gray-500">
                        {saving.status === "approved" ? "Approved By" : "Rejected By"}
                      </label>
                      <p className="text-lg">{saving.approvedByName}</p>
                    </div>
                  </>
                )}
              </div>
          ...
          {saving.notes && (
            <Card>
              ...
              <CardContent>
                <p className="text-gray-700">{saving.notes}</p>
              </CardContent>
            </Card>
          )}
        </div>
        ...
        <div className="flex justify-between items-center pt-6 border-t">
          ...
          {saving.status === "pending" && (
            <div className="flex space-x-2">
              <Button
                onClick={handleRejectSaving}
                variant="outline"
                className="text-red-600 hover:text-red-700 border-red-300 hover:border-red-400"
                disabled={rejectSavingMutation.isPending}
                data-testid="button-reject-saving"
              >
                <XCircle className="mr-2 h-4 w-4" />
                {rejectSavingMutation.isPending ? "Rejecting..." : "Reject"}
              </Button>
              <Button
                onClick={handleApproveSaving}
                className="bg-emerald-600 hover:bg-emerald-700"
                disabled={approveSavingMutation.isPending}
                data-testid="button-approve-saving"
              >
                <CheckCircle className="mr-2 h-4 w-4" />
                {approveSavingMutation.isPending ? "Approving..." : "Approve"}
              </Button>
            </div>
          )}
        </div>
